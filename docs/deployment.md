...
One local sample on macOS arm64 with Node.js 25.9.0, `https://example.com/`, 100 requests and concurrency 4: 100 succeeded in 0.74 seconds, p50 25.7 ms, p95 35.1 ms, 75,600 response bytes, sampled peak RSS 55.2 MiB, and sampled average process CPU 9.5%. The built relay directory was 60 KiB, the SDK output directory 36 KiB, and the packed SDK tarball 4,871 bytes. In a [Linux amd64 CI run](https://github.com/tinyaifoundation/tiny-browser/actions/runs/36189531475), the built container image measured 167,391,711 bytes (159.6 MiB), including the Node.js base image. These are short samples, not capacity or billing estimates.
